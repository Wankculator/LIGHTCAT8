...
// Initialize the tested RGB Integration system
const rgbIntegrationController = require('./controllers/rgbIntegrationController');
const rgbIntegrationRoutes = require('./routes/rgbIntegrationRoutes');
...
// Use the proven RGB Integration routes
app.use('/api/rgb', rgbIntegrationRoutes);
...
app.get('/api/rgb/invoice/:invoiceId/status', async (req, res) => {
    ...
    // Return appropriate status based on invoice state
    if (invoice.status === 'delivered' && invoice.consignment) {
    ...
    } else if (invoice.status === 'distribution_failed') {
        res.json({
            success: false,
            status: 'distribution_failed',
            error: invoice.error || 'Token distribution failed. Please contact support.',
            invoice: {
                status: 'distribution_failed',
                amount: invoice.amount,
                batchCount: invoice.batchCount
        ...
        res.json({
            ...
            message: invoice.status === 'pending' ? 'Waiting for payment...' : 'Processing payment...',
...
// BTCPay webhook is now handled by the tested rgbIntegrationRoutes at /api/rgb/webhook/btcpay
// Legacy webhook endpoint for compatibility
app.post('/api/webhooks/btcpay', (req, res) => {
    logger.info('Legacy webhook received, forwarding to RGB integration');
    // Forward to the tested webhook handler
    req.url = '/api/rgb/webhook/btcpay';
    rgbIntegrationRoutes(req, res);
});
