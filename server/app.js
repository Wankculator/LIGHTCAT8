            ...
            res.on('end', () => {
                try {
                    ...
                    if (res.statusCode >= 200 && res.statusCode < 300) {
                        ...
                        reject(new Error(`BTCPay error: ${result.message || body}`));
                    }
                } catch (e) {
...
app.post('/api/rgb/invoice', async (req, res) => {
    try {
        ...
        // Allow demo purchase of 1 batch without tier
        if (!tier) {
            if (batchCount !== 1) {
                return res.status(400).json({
                    success: false,
                    error: 'Without playing the game, you can only purchase 1 batch. Play to unlock up to 30 batches!'
                });
            }
            // Allow 1 batch demo purchase
        }
        ...
        if (BTCPAY_API_KEY) {
            try {
                ...
                logger.info('BTCPay invoice created:', btcpayResponse.id);
                btcpayInvoiceId = btcpayResponse.id;
                
                // Get payment methods to find Lightning invoice
                try {
                    const paymentMethods = await btcpayRequest(
                        `/api/v1/stores/${BTCPAY_STORE_ID}/invoices/${btcpayInvoiceId}/payment-methods`,
                        'GET'
                    );
                    
                    const lightningMethod = paymentMethods.find(m => m.paymentMethod === 'BTC-LN');
                    paymentRequest = lightningMethod?.destination;
                    
                    if (!paymentRequest) {
                        // Fallback to checkout link if no Lightning invoice
                        paymentRequest = btcpayResponse.checkoutLink;
                        logger.warn('No Lightning invoice found, using checkout link');
                    }
                } catch (err) {
...
app.get('/api/rgb/invoice/:invoiceId/status', async (req, res) => {
    ...
    res.json({
        ...
        distributionStatus: invoice.distributionStatus,
        invoice: {
            id: invoiceId,
            batchCount: invoice.batchCount,
            tokenAmount: invoice.batchCount * 700,
...
// Import REAL RGB distribution service with wallet
const rgbDistributionService = require('./services/rgbRealDistribution');

// BTCPay webhook endpoint
app.post('/api/webhooks/btcpay', async (req, res) => {
    ...
    if (req.body.type === 'InvoiceSettled') {
        ...
        if (metadata && metadata.orderId) {
            ...
            if (invoice) {
                ...
                try {
                    logger.info('🚀 Starting token distribution for:', metadata.orderId);
                    
                    const distribution = await rgbDistributionService.distributeTokens({
                        invoiceId: metadata.orderId,
                        rgbInvoice: invoice.rgbInvoice,
                        batchCount: invoice.batchCount,
                        paymentHash: req.body.paymentHash || 'btcpay-' + Date.now(),
                        buyerEmail: invoice.email
                    });
                    
                    // Store consignment for download
                    invoice.consignment = distribution.consignment;
                    invoice.distributionStatus = 'completed';
                    invoice.distributedAt = new Date().toISOString();
                    invoiceStore.set(metadata.orderId, invoice);
                    
                    logger.info('✅ Tokens distributed successfully!', {
                        invoiceId: metadata.orderId,
                    ...
                    invoice.distributionStatus = 'failed';
                    invoice.distributionError = error.message;
                    invoiceStore.set(metadata.orderId, invoice);
                }
            }
        }
