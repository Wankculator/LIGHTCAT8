...
class ValidationService {
  // RGB Invoice Validation - Updated to accept multiple RGB formats
  validateRGBInvoice(rgbInvoice) {
    ...
    // Trim whitespace
    rgbInvoice = rgbInvoice.trim();

    // Check basic format - must start with 'rgb' (can be rgb:, rgb1, etc.)
    if (!rgbInvoice.startsWith('rgb')) {
      return { isValid: false, error: 'Invalid RGB invoice format. Must start with "rgb"' };
    }

    // Accept multiple RGB formats:
    // 1. rgb:utxob:... (classic format)
    // 2. rgb1q... (bech32-like format)
    // 3. rgb:2... or rgb:u... (asset ID format)
    // 4. Complex format with slashes: rgb:utxob:.../RGB20/...+bc:...
    // 5. IRIS wallet format with tildes and endpoints: rgb:~/~/~/bc:utxob:...?expiry=...&endpoints=...
    const validPatterns = [
      /^rgb:utxob:[\w\-+/:.]+$/,  // Classic with special chars
      /^rgb1[qpzry9x8gf2tvdw0s3jn54khce6mua7l]+$/,  // Bech32-like
      /^rgb:[2u][\w\-]+$/,  // Asset ID format
      /^rgb:[\w\-+/:.]+$/,   // General flexible format
      /^rgb:.*$/  // Accept ANY format starting with rgb: (including IRIS wallet format)
    ];

    const isValidFormat = validPatterns.some(pattern => pattern.test(rgbInvoice));
  ...
  validateBatchCount(batchCount, tier = null) {
    ...
    const limits = {
      bronze: 5,    // Bronze: 5 batches
      silver: 8,    // Silver: 8 batches
      gold: 10,     // Gold: 10 batches
