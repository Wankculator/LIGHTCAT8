...
class RGBService {
  ...
  async generateConsignment(options) {
    const { rgbInvoice, amount, invoiceId, batchCount } = options;
    ...
    try {
      ...
      // Use the new distribution service for production
      const rgbDistributionService = require('./rgbDistributionService');
      
      // Calculate token amount
      const tokenAmount = (batchCount || 1) * 700; // 700 tokens per batch
      
      // Distribute tokens using the new service
      const result = await rgbDistributionService.distributeTokens({
        invoiceId,
        rgbInvoice,
        batchCount: batchCount || Math.floor(amount / 700),
        paymentHash: options.paymentHash || '',
        buyerEmail: options.buyerEmail || ''
      });

      if (!result.success || !result.consignment) {
        throw new Error('Distribution service failed to generate consignment');
      }

      logger.info('Consignment generated successfully via distribution service:', {
        invoiceId,
        tokenAmount: result.tokenAmount,
        consignmentSize: result.consignment.length
      });
      ...
      // Fallback to mock mode if configured
      if (this.mockMode) {
        logger.warn('Falling back to mock consignment generation');
        return this.generateMockConsignment(options);
      }
  ...
  async generateMockConsignment(options) {
    const { rgbInvoice, amount, invoiceId } = options;
    
    // Create mock consignment
    const mockConsignment = Buffer.concat([
      Buffer.from('RGB_MOCK_CONSIGNMENT_V1'),
      Buffer.from(invoiceId || 'no-id'),
      Buffer.from(rgbInvoice),
      Buffer.alloc(8),
      Buffer.from(amount.toString())
    ]);
    
    return mockConsignment.toString('base64');
  }
}

// Export the class for testing, and an instance for normal use
module.exports = RGBService;
module.exports.default = new RGBService();