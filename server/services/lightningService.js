...
const BTCPayVoltageService = require('./btcpayVoltageService');

class LightningService {
  constructor() {
    // Check if BTCPay is configured first
    if (process.env.USE_BTCPAY === 'true' && process.env.BTCPAY_API_KEY) {
      logger.info('Using BTCPay service for Lightning');
      this.btcpayService = new BTCPayVoltageService();
      this.useBTCPay = true;
      this.useVoltage = false;
      
      // Set up event listeners for BTCPay
      this.btcpayService.on('invoice:paid', (invoice) => {
        logger.info('Invoice paid via BTCPay:', invoice.invoiceId);
      });
    }
    // Use Voltage Lightning service if configured
    else if (process.env.VOLTAGE_NODE_URL || process.env.LIGHTNING_MACAROON_PATH) {
      logger.info('Using Voltage Lightning service');
      this.voltageService = new VoltageLightningService();
      this.useVoltage = true;
      this.useBTCPay = false;
  ...
  async createInvoice(params) {
    try {
      // Use BTCPay service if configured
      if (this.useBTCPay) {
        const btcPayInvoice = await this.btcpayService.createInvoice(params);
        
        // Map BTCPay response to expected format
        return {
          id: btcPayInvoice.invoiceId,
          payment_request: btcPayInvoice.lightningInvoice, // This contains the BOLT11 invoice
          payment_hash: btcPayInvoice.invoiceId, // Use invoice ID as hash for BTCPay
          amount_sat: btcPayInvoice.amount,
          memo: params.description || params.memo,
          expires_at: btcPayInvoice.expiresAt,
          created_at: btcPayInvoice.createdAt || new Date(),
          checkoutUrl: btcPayInvoice.checkoutUrl
        };
      }
      
      // Use Voltage service if available
      if (this.useVoltage) {
        const { amount, memo, expiry = 3600 } = params;
  ...
  createMockInvoice(amount, memo, expiry) {
    ...
    logger.warn('Using MOCK invoice - BTCPay not properly configured or failed');
