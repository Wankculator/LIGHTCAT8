...
class RGBPaymentController {
  ...
  async createInvoice(req, res) {
    try {
      ...
      // CRITICAL: Check if tier is provided - allow 1 batch demo without tier
      if (!tier) {
        // Allow demo purchase of 1 batch without tier
        if (batchCount === 1) {
          logger.info(`Allowing demo purchase: 1 batch without tier`);
          // Continue with purchase
        } else {
          logger.warn(`Rejecting purchase: ${batchCount} batches requested without tier - only 1 allowed`);
          return res.status(400).json({
            success: false,
