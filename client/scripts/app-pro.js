    ...
    if (url.includes('/api/payments/create-invoice') || url.includes('create-invoice')) {
        ...
        return Promise.resolve({
            ...
            json: () => Promise.resolve({
                invoiceId: 'RGB-' + Date.now() + '-' + crypto.randomBytes(5).toString('hex'),
...
function updateElementSmooth(id, newValue) {
    ...
    if (difference !== 0) {
        ...
        const interval = this.interval2 = setInterval(() => {
            step++;
...
function showNotification(message, type = 'success') {
    ...
    this.timeout3 = setTimeout(() => {
        ...
        this.timeout4 = setTimeout(() => notif.remove(), 300);
    }, 3000);
}
...
// Check tier on load
window.SafeEvents.on(document, 'DOMContentLoaded', updateTierDisplay);
