...
class LitecatGameEngine {
  ...
  setupEventListeners() {
    // Keyboard events
    window.SafeEvents.on(window, 'keydown', (e) => {
    ...
    window.SafeEvents.on(window, 'keyup', (e) => {
      this.keys[e.code] = false;
    });
    
    // Touch events for mobile
    this.canvas.addEventListener('touchstart', this.handleTouch.bind(this));
    this.window.SafeEvents.on(canvas, 'touchmove', this.handleTouch.bind(this));
  ...
  generateStars(count) {
    ...
    for (let i = 0; i < count; i++) {
      stars.push({
        x: gameRandom() * this.canvas.width,
        y: gameRandom() * this.canvas.height,
        size: gameRandom() * 2 + 0.5,
        speed: gameRandom() * 0.5 + 0.1,
        opacity: gameRandom() * 0.8 + 0.2
      });
    }
    return stars;
  }
  ...
  updateStars() {
    this.stars.forEach(star => {
      ...
      if (star.y > this.canvas.height) {
        star.y = -5;
        star.x = gameRandom() * this.canvas.width;
      }
    });
  }
  ...
  spawnEnemies() {
    ...
    if (this.enemySpawnTimer > spawnRate) {
      ...
      const type = enemyTypes[Math.floor(gameRandom() * enemyTypes.length)];
      
      this.enemies.push({
        x: gameRandom() * (this.canvas.width - 30),
        y: -30,
        width: 30,
        height: 30,
        speed: 1 + gameRandom() * 2,
  ...
  spawnPowerUps() {
    ...
    if (this.powerUpSpawnTimer > 8000 && gameRandom() < 0.3) {
      const powerUpTypes = ['multishot', 'shield', 'lightning'];
      const type = powerUpTypes[Math.floor(gameRandom() * powerUpTypes.length)];
      
      this.powerUps.push({
        x: gameRandom() * (this.canvas.width - 20),
        y: -20,
        size: 20,
        type: type,
        rotation: 0,
