...
class LitecatGame {
    ...
    createStarfield() {
        ...
        for (let i = 0; i < 150; i++) {
            stars.push({
                x: gameRandom() * this.width,
                y: gameRandom() * this.height,
                size: gameRandom() * 2 + 0.5,
                speed: gameRandom() * 0.5 + 0.1,
                twinkle: gameRandom() * Math.PI * 2
            });
        }
        return stars;
    }
    ...
    generateLightningPath(x1, y1, x2, y2) {
        ...
        for (let i = 0; i <= steps; i++) {
            const t = i / steps;
            const x = x1 + (x2 - x1) * t + (gameRandom() - 0.5) * 20;
            const y = y1 + (y2 - y1) * t;
            segments.push({ x, y });
        }
    ...
    spawnWave() {
        ...
        for (let i = 0; i < waveSize; i++) {
            if (gameRandom() > 0.3) {
                // Spawn BTC coin
                this.btcCoins.push({
                    x: gameRandom() * (this.width - 40) + 20,
                    y: -50 - (i * 60),
                    size: 30,
                    speed: 2 + gameRandom() * 2,
                    rotation: gameRandom() * Math.PI * 2,
                    rotationSpeed: 0.05 + gameRandom() * 0.05,
                    bobPhase: gameRandom() * Math.PI * 2,
                    value: gameRandom() > 0.9 ? 10 : 5 // rare golden coins worth more
                });
            } else {
                // Spawn enemy
                this.enemies.push({
                    ...
                    width: 40,
                    height: 40,
                    speed: 3 + gameRandom() * 2,
                    type: gameRandom() > 0.7 ? 'zigzag' : 'normal',
                    health: 1
                });
            }
        }
            ...
            if (bolt.power > 1.5) {
                bolt.segments.forEach((seg, i) => {
                    if (i % 3 === 0 && i > 0) {
                        this.ctx.beginPath();
                        this.ctx.moveTo(seg.x, seg.y);
                        this.ctx.lineTo(seg.x + (gameRandom() - 0.5) * 30, seg.y - 20);
                        this.ctx.stroke();
                    }
                });
            }
