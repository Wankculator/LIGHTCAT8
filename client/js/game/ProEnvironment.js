...
export class ProEnvironment {
    ...
    createCityscape() {
        ...
        for (let i = 0; i < buildingCount; i++) {
            const width = 5 + gameRandom() * 15;
            const height = 20 + gameRandom() * 60;
            const depth = 5 + gameRandom() * 15;
                ...
                for (let col = 0; col < windowCols; col++) {
                    if (gameRandom() > 0.3) {
                        ...
                        windowLight.position.set(
                            ...
                            row * 5 + 5,
                            building.position.z + (depth/2 + 0.1) * (gameRandom() > 0.5 ? 1 : -1)
                        );
    ...
    createAtmosphere() {
        ...
        for (let i = 0; i < particleCount; i++) {
            positions[i * 3] = (gameRandom() - 0.5) * 200;
            positions[i * 3 + 1] = gameRandom() * 100;
            positions[i * 3 + 2] = (gameRandom() - 0.5) * 200;
            
            // Yellow and white particles
            const isYellow = gameRandom() > 0.7;
            colors[i * 3] = isYellow ? 1 : 0.5;
            colors[i * 3 + 1] = isYellow ? 1 : 0.5;
            colors[i * 3 + 2] = isYellow ? 0 : 0.5;
            
            sizes[i] = gameRandom() * 2 + 0.5;
        }
    ...
    update(deltaTime) {
        ...
        this.lightningStrikes.forEach(strike => {
            ...
            if (strike.nextStrike <= 0) {
                ...
                strike.mesh.scale.y = 1 + gameRandom() * 0.5;
                ...
                strike.nextStrike = 5 + gameRandom() * 15;
            }
        });
