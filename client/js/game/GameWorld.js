...
export class GameWorld {
    ...
    createBuildings() {
        ...
        for (let i = 0; i < 20; i++) {
            const width = 5 + gameRandom() * 10;
            const height = 10 + gameRandom() * 30;
            const depth = 5 + gameRandom() * 10;
            
            const buildingGeometry = new THREE.BoxGeometry(width, height, depth);
            const building = new THREE.Mesh(buildingGeometry, buildingMaterial);
            
            building.position.x = (gameRandom() - 0.5) * 150;
            building.position.z = (gameRandom() - 0.5) * 150;
            ...
            for (let j = 0; j < windowCount; j++) {
                ...
                windowLight.position.set(
                    building.position.x + (gameRandom() - 0.5) * width,
                    building.position.y - height/2 + j * 3 + 2,
                    building.position.z + (gameRandom() - 0.5) * depth
                );
    ...
    createParticles() {
        ...
        for (let i = 0; i < particleCount * 3; i += 3) {
            positions[i] = (gameRandom() - 0.5) * 200;
            positions[i + 1] = gameRandom() * 50;
            positions[i + 2] = (gameRandom() - 0.5) * 200;
            
            // Yellow/electric colors
            colors[i] = 1;
            colors[i + 1] = 0.9 + gameRandom() * 0.1;
            colors[i + 2] = 0.2 + gameRandom() * 0.3;
        }
