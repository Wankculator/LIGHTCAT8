...
export class ProGame {
    ...
    init() {
        try {
            // Check if mobile and get performance config
            const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
            const perfConfig = window.MOBILE_PERFORMANCE_CONFIG || {};
            
            // Renderer with mobile-friendly settings
            this.renderer = new THREE.WebGLRenderer({ 
                canvas: this.canvas,
                antialias: !isMobile && !perfConfig.disableAntialiasing,
            ...
            this.renderer.setPixelRatio(Math.min(window.devicePixelRatio, perfConfig.pixelRatio || (isMobile ? 1.5 : 2))); // Use config or default
            this.renderer.shadowMap.enabled = !isMobile && !perfConfig.disableShadows; // Disable shadows on mobile
            ...
            this.usePostProcessing = !isMobile && !perfConfig.disablePostProcessing;
    ...
    createLightningSystem() {
        ...
        for (let i = 0; i < 20; i++) {
            ...
            bolt.position.set(
                (gameRandom() - 0.5) * 60,  // Closer spawn radius
                gameRandom() * 15 + 10,     // Lower height range
                (gameRandom() - 0.5) * 60
            );
            ...
            bolt.userData = {
                ...
                fallSpeed: 3 + gameRandom() * 2,
                rotationSpeed: 2 + gameRandom() * 2,
                floatOffset: gameRandom() * Math.PI * 2,
                collected: false
            };
    ...
    setupControls() {
        // Keyboard
        window.SafeEvents.on(document, 'keydown', (e) => {
        ...
        if (!this.input.isMobile) {
            ...
            // Use SafeEvents if available, otherwise use standard addEventListener
            if (window.SafeEvents) {
                window.SafeEvents.on(this.canvas, 'click', clickHandler);
            } else {
        ...
        // Use SafeEvents if available, otherwise use standard addEventListener
        if (window.SafeEvents) {
            window.SafeEvents.on(document, 'mousemove', mouseMoveHandler);
        } else {
    ...
    setupMobileControls() {
        ...
        window.SafeEvents.on(joystick, 'touchstart', (e) => {
            joystickActive = true;
            handleTouch(e);
        });
        ...
        window.SafeEvents.on(joystick, 'touchend', () => {
        ...
        // Jump button
        window.SafeEvents.on(jumpBtn, 'touchstart', (e) => {
            e.preventDefault();
            this.jump();
        });
    ...
    endGame() {
        console.log('[ProGame] endGame called, isPlaying:', this.isPlaying);
        
        if (!this.isPlaying) {
            console.log('[ProGame] Game already ended, returning');
            return;
        }
        ...
        console.log('[ProGame] Game ending with score:', finalScore);
        
        // Try multiple methods to show game over
        setTimeout(() => {
            console.log('[ProGame] Attempting to show game over screen...');
            
            // Method 1: Use game UI
            if (window.game && window.game.ui && window.game.ui.showGameOver) {
                console.log('[ProGame] Calling ui.showGameOver');
                window.game.ui.showGameOver(finalScore);
            }
            // Method 2: Use main game endGame
            else if (window.game && window.game.endGame) {
                console.log('[ProGame] Calling game.endGame');
                window.game.endGame();
            }
            // Method 3: Force it
            else {
                console.log('[ProGame] Forcing game over display');
    ...
    collectLightning(bolt) {
        ...
        setTimeout(() => {
            ...
            bolt.userData.fallSpeed = 3 + gameRandom() * 2;
        }, 2000);
    }
    ...
    createCollectionEffect(position) {
        ...
        for (let i = 0; i < burstCount; i++) {
            ...
            const speed = 3 + gameRandom() * 5;
            const upSpeed = gameRandom() * 5 + 3;
            ...
            particle.life = 0.8;
            particle.rotationSpeed = gameRandom() * 5;
