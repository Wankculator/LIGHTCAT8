...
export class LightningRain {
    ...
    createRainEffect() {
        ...
        for (let i = 0; i < this.particleCount; i++) {
            rainPositions[i * 3] = (gameRandom() - 0.5) * this.bounds.x;
            rainPositions[i * 3 + 1] = gameRandom() * this.bounds.y;
            rainPositions[i * 3 + 2] = (gameRandom() - 0.5) * this.bounds.z;
            
            rainVelocities[i] = 0.5 + gameRandom() * 0.5;
        }
    ...
    createCollectibleLightning() {
        ...
        for (let i = 0; i < this.collectibleCount; i++) {
            ...
            lightning.position.set(
                (gameRandom() - 0.5) * this.bounds.x * 0.8,
                gameRandom() * 10 + 10,
                (gameRandom() - 0.5) * this.bounds.z * 0.8
            );
            
            // Random rotation
            lightning.rotation.z = gameRandom() * Math.PI;
            ...
            lightning.userData = {
                collected: false,
                glowLight: glowLight,
                fallSpeed: 2 + gameRandom() * 2,
                rotationSpeed: gameRandom() * 0.05,
    ...
    update(deltaTime) {
        ...
        for (let i = 0; i < this.particleCount; i++) {
            ...
            if (positions[i * 3 + 1] < 0) {
                positions[i * 3] = (gameRandom() - 0.5) * this.bounds.x;
                positions[i * 3 + 1] = this.bounds.y;
                positions[i * 3 + 2] = (gameRandom() - 0.5) * this.bounds.z;
            }
        }
        ...
        this.lightningBolts.forEach(lightning => {
            if (!lightning.userData.collected) {
                ...
                if (lightning.position.y < 0.5) {
                    ...
                    lightning.position.x = (gameRandom() - 0.5) * this.bounds.x * 0.8;
                    lightning.position.z = (gameRandom() - 0.5) * this.bounds.z * 0.8;
                }
            }
        });
    }
