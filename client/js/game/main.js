...
class LightcatGame {
    ...
    setupEventListeners() {
        ...
        if (window.SafeEvents && window.SafeEvents.on) {
        ...
        if (playAgainBtn) {
            window.SafeEvents.on(playAgainBtn, 'click', () => {
                this.resetGame();
            });
        }
        ...
        if (twitterFollowBtn) {
            window.SafeEvents.on(twitterFollowBtn, 'click', () => {
        ...
        if (unlockTierBtn) {
            window.SafeEvents.on(unlockTierBtn, 'click', () => {
    ...
    startGame() {
        ...
        // Set game fully loaded flag for fixes to use
        window.gameFullyLoaded = true;
        console.log('[LightcatGame] Game fully loaded and started');
    ...
    proceedToPurchase() {
        ...
        if (isDesktop) {
            // Desktop: Try to redirect the parent window to prevent iframe nesting
            try {
                // First try to access parent window
                ...
                // Cross-origin restriction, try window.top
                try {
                    if (window.top && window.top !== window) {
                        window.top.location.href = '/#purchase?tier=' + tier.toLowerCase();
                    } else {
                        // Last resort: open in new tab to escape iframe
                        window.open('/#purchase?tier=' + tier.toLowerCase(), '_blank');
                        alert('Your ' + tier.toUpperCase() + ' tier has been unlocked! Check the new tab to claim your tokens.');
                    }
                } catch (e2) {
                    // Complete cross-origin block, open in new tab
                    window.open('/#purchase?tier=' + tier.toLowerCase(), '_blank');
                    alert('Your ' + tier.toUpperCase() + ' tier has been unlocked! Check the new tab to claim your tokens.');
                }
            }
        } else {
            // Mobile: Keep existing redirect behavior
            try {
...
window.SafeEvents.on(window, 'DOMContentLoaded', () => {
    try {
        // THREE is imported as a module, not a global
        console.log('[LightcatGame] Starting game initialization...');
