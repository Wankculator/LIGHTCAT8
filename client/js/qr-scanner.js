...
class LightcatQRScanner {
    ...
    initializeEventListeners() {
        ...
        if (scanBtn) {
            window.SafeEvents.on(scanBtn, 'click', () => this.openScanner());
        }
        ...
        if (closeBtn) {
            window.SafeEvents.on(closeBtn, 'click', () => this.closeScanner());
        }
        ...
        if (uploadBtn && fileInput) {
            window.SafeEvents.on(uploadBtn, 'click', () => fileInput.click());
        ...
        this.window.SafeEvents.on(modal, 'click', this.clickHandler);
    }
    ...
    openScanner() {
        ...
        // Clear any existing scanner and reset container
        if (this.scanner) {
    ...
    async startScanning() {
        try {
            ...
            // Configure scanner - minimal and clean
            const config = {
                fps: 10,
            ...
            // Hide camera selector completely on mobile
            setTimeout(() => {
                const cameraSelect = document.querySelector('#qrReaderDiv select');
                if (cameraSelect) {
                    cameraSelect.style.display = 'none';
                }
                // Also hide any camera selection UI elements
                const cameraSelectionUI = document.querySelectorAll('#qrReaderDiv__camera_selection, #qrReaderDiv__dashboard_section_csr');
                cameraSelectionUI.forEach(el => {
